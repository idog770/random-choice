...
Sf.pack()
# Star Wars
SwVar = tkinter.IntVar()
Sw = tkinter.Checkbutton(root, text="Star Wars", variable=SwVar, command=lambda: game(SwVar, "Star Wars"))
Sw.pack()
# VR
...
def randomization():
    ...
    if rgame == "Playstation 5":
        if count == 1:
            ...
            GOW = tkinter.Checkbutton(root, text="God Of War", variable=GOWVar, command=lambda: game(GOWVar,
                                                                                                     "God Of "                                                                                                "War"))
            HZD = tkinter.Checkbutton(root, text="Horizon Zero Dawn", variable=HZDVar,
                                      command=lambda: game(HZDVar, "Horizon Zero Dawn"))
            SDM = tkinter.Checkbutton(root, text="Spider-Man", variable=SDMVar,
